...
devices = {}
#devices["dev2"] = ["/dev/nvme0n1","/dev/nvme1n1"]
devices["ns2"] = ["/dev/nvme2n1:/dev/nvme2n2"]

#Define workload parameters
workload_type = ["randread"]

#queue_depth = [2**i for i in range(9)]
queue_depth = [1, 32, 64, 128, 256]
#queue_depth = [i for i in range(2,128,2)]
req_size = ["4k"]
number_of_process = [1, 2, 4, 8, 10, 11, 12, 14, 16]
iouring_type = ["iou","ioup"]
                ...
                for qd in queue_depth:
                    for np in number_of_process:
                        for io in iouring_type:
                            parameters = {}
                            for i,j in enumerate(devices[dt]):
                                parameters["DEVICE"+str(i)] = j
